...
export const MyCoursesPage = () => {
  ...
  const courses = useMemo(() => {
    return data?.length ? data.filter((c: CoursesResponseItem) => c.authorId === user?.userId) : [];
  }, [data, user?.userId]);
  return (
    <div className="space-y-8 py-8 sm:py-10">
      <section className="rounded-[40px] bg-slate-950 p-8 text-white shadow-[0_45px_120px_-65px_rgba(15,23,42,1)] sm:p-10">
        ...
        <p className="mt-4 max-w-2xl text-base leading-7 text-slate-300">
          Здесь автор сможет видеть свои черновики, опубликованные мини-курсы, карточки статусов и
          базовую аналитику.
        </p>
        <div className="mt-6">
      ...
      <section className="grid gap-4 md:grid-cols-3">
        ...
        <div className="rounded-[32px] bg-teal-400 p-6 text-slate-950 shadow-[0_35px_110px_-65px_rgba(45,212,191,0.95)]">
          ...
          <p className="mt-2 text-4xl font-semibold">{(courses?.length || 0) * 20} эфира</p>
        </div>
      </section>
